"use client";

import { useTheme } from "next-themes";
import { memo } from "react";

export const ThemeIcon = memo(function ThemeIcon() {
  const { theme, setTheme } = useTheme();

  const handleTheme = () => {
    theme === "dark" ? setTheme("light") : setTheme("dark");
  };

  return (
    <div>
      <svg
        width="36"
        height="37"
        viewBox="0 0 36 37"
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
        onClick={() => handleTheme()}
        className="relative left-[1px] mr-[4px] cursor-pointer fill-[#2D384B] hover:fill-[#fff] md:hidden"
      >
        <path d="M0 12.5C0 5.87258 5.37258 0.5 12 0.5H24C30.6274 0.5 36 5.87258 36 12.5V24.5C36 31.1274 30.6274 36.5 24 36.5H12C5.37258 36.5 0 31.1274 0 24.5V12.5Z" />
        <path
          d="M22.9998 10.167L23.5147 11.1966C23.7359 11.6391 23.8465 11.8604 23.9943 12.0521C24.1254 12.2222 24.278 12.3747 24.4481 12.5059C24.6398 12.6536 24.861 12.7643 25.3035 12.9855L26.3332 13.5003L25.3035 14.0152C24.861 14.2364 24.6398 14.347 24.4481 14.4948C24.278 14.6259 24.1254 14.7784 23.9943 14.9486C23.8465 15.1403 23.7359 15.3615 23.5147 15.804L22.9998 16.8337L22.485 15.804C22.2638 15.3615 22.1531 15.1403 22.0054 14.9486C21.8742 14.7784 21.7217 14.6259 21.5516 14.4948C21.3599 14.347 21.1386 14.2364 20.6962 14.0152L19.6665 13.5003L20.6962 12.9855C21.1386 12.7643 21.3599 12.6536 21.5516 12.5059C21.7217 12.3747 21.8742 12.2222 22.0054 12.0521C22.1531 11.8604 22.2638 11.6391 22.485 11.1966L22.9998 10.167Z"
          stroke="#98A2B3"
          strokeWidth="1.5"
          strokeLinecap="round"
          strokeLinejoin="round"
        />
        <path
          d="M25.4998 19.6581C24.4073 21.5745 22.3453 22.8666 19.9815 22.8666C16.4757 22.8666 13.6338 20.0246 13.6338 16.5189C13.6338 14.1549 14.9261 12.0928 16.8428 11.0003C12.8163 11.3821 9.6665 14.7728 9.6665 18.8991C9.6665 23.2812 13.2189 26.8337 17.6011 26.8337C21.7272 26.8337 25.1178 23.6842 25.4998 19.6581Z"
          stroke="#98A2B3"
          strokeWidth="1.5"
          strokeLinecap="round"
          strokeLinejoin="round"
        />
      </svg>
    </div>
  );
});
